      ...
      <div className="sticky top-0 h-screen flex items-center overflow-hidden relative">
        ...
        <motion.div style={{ x }} className="relative flex gap-6 md:gap-10 px-6 md:px-20">
          {services.map((service, index) => (
            <div
              key={index}
              className="group relative w-[85vw] xs:w-[80vw] md:w-[600px] h-[55vh] md:h-[60vh] bg-[var(--color-bg-secondary)] border border-[var(--color-border-primary)] rounded-3xl overflow-hidden backdrop-blur-sm hover:border-[var(--color-brand-orange)] transition-all duration-500 flex flex-col justify-center items-center p-6 md:p-8 text-center"
            >
              ...
              <motion.div
                className="mb-4 xs:mb-8 p-4 xs:p-6 rounded-full bg-white/5 text-[var(--color-brand-orange)] group-hover:scale-110 transition-transform duration-300 shadow-[0_0_20px_rgba(255,159,28,0.2)]"
              >
                {service.icon}
              </motion.div>

              <h3 className="text-2xl xs:text-3xl font-brand font-bold text-[var(--color-text-primary)] mb-3 xs:mb-4 group-hover:text-[var(--color-brand-orange)] transition-colors">
                {service.title}
              </h3>

              <p className="text-[var(--color-text-secondary)] font-inter text-sm xs:text-lg leading-relaxed max-w-md">
                {service.description}
              </p>
